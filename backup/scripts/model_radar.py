...
class Retrieval:
    ...
    def model_inverse_03a(
        self, y1, W1, bounds, name="tmp", init_weights=[4, 1], seed=None
    ):
        """
    ...
    def model_inverse_03a_v1(
        ...
        def model_inversion(instance):
            ...
            param0 = result.x
            costs = np.full(param0.shape, result.fun)

            y_hat0 = self.model_03a_fun(W, param0)
            return y_hat0, param0, costs

        params = []
        y_hat = []
        costs = []
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for a, b, c in executor.map(model_inversion, range(y1.shape[0])):
                y_hat.append(a.flatten())
                params.append(b)
                costs.append(c)
        ...
        params = np.array(params)
        y_hat = np.array(y_hat)
        return params, y_hat, np.mean(costs)
...
def scatter_plot_agb_retrieval0(
    ...
    if test_mask is None:
        ...
        z1_test, W1_test, mask_ws1_test = mdl00.inversion_setup(
            sar_list, agb_file=agb_file, mask_file=test_mask, n_1=nwin_size
        )
...
def colormap_plot_agb_prediction(
    ...
    for k in range(len(out_radar_list) // 2):
        ...
        # reload the image
        ds = gdal.Open(agb_name, gdal.GA_ReadOnly)
        array0 = ds.GetRasterBand(1).ReadAsArray()

        # im = ax1[k].imshow(array0, cmap="gist_earth_r", vmin=np.quantile(W_mean, 0.01), vmax=np.quantile(W_mean, 0.99))
        im = ax1[k].imshow(array0, cmap="gist_earth_r", vmin=0, vmax=100)
